  ...
  async up(queryInterface, Sequelize) {
    ...
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      if (existingResult.length === 0) {
        ...
        console.log('Assigned ATTENDANCE_VIEW_TEAM permission to MANAGER role');
      } else {
        console.log('ATTENDANCE_VIEW_TEAM permission already assigned to MANAGER role');
      }
    });
  },

  async down(queryInterface, Sequelize) {
    // Remove ATTENDANCE_VIEW_TEAM permission from MANAGER (if it was added by this migration)
    await queryInterface.sequelize.transaction(async (transaction) => {
      const [roleResult] = await queryInterface.sequelize.query(`
        SELECT id FROM role WHERE name = 'MANAGER'
      `, { transaction });
      ...
      console.log('Removed ATTENDANCE_VIEW_TEAM permission from MANAGER role');
    });
  }
};
