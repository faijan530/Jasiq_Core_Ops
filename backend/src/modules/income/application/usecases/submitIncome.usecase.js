...
export async function submitIncomeUsecase(pool, { id, actorId, requestId, overrideReason }) {
  ...
  return withTransaction(pool, async (client) => {
    ...
    assertTransition({ fromStatus: current.status, toStatus: INCOME_STATUS.SUBMITTED });
    ...
    const updated = await updateIncomeState(client, {
      id,
      status: INCOME_STATUS.SUBMITTED,
