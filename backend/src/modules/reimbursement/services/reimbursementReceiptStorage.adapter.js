...
export async function storeReimbursementReceipt({ reimbursementId, fileName, contentType, fileBase64 }) {
  ...
  const relDir = path.join('storage', 'reimbursement_receipts', String(reimbursementId));
  ...
  return {
    storageKey: `reimbursement_receipts/${reimbursementId}/${base}_${rand}.${ext}`,
    fileSize: buf.length,
    relPath: relFile.replace(/\\/g, '/')
  };
}

export async function readReimbursementReceiptFromStorage({ relPath }) {
