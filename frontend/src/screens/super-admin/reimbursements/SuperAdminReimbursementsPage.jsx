...
export function SuperAdminReimbursementsPage() {
  ...
  }, [page, statusFilter, month, canView]);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-slate-600 mt-1">Global view (role/permission gated).</p>
      </div>
      ...
      {items.length === 0 ? (
        <EmptyState title="No data" description="No reimbursements matched your filters." />
      ) : (
        <ReimbursementTable rows={items} loading={false} empty="No reimbursements" onRowClick={(r) => navigate(`/super-admin/reimbursements/${r.id}`)} />
      )}
