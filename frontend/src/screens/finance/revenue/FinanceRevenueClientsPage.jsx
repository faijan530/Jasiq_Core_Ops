...
export function FinanceRevenueClientsPage() {
  ...
  async function load() {
    try {
      setStatus('loading');
      setError(null);
      const payload = await incomeService.listClients({ page: 1, size: 200 });
  ...
  if (status === 'loading') return <LoadingState message="Loading clients…" />;

  if (status === 'error') {
    if (error?.status === 403) {
      return <ForbiddenState error={{ message: 'Forbidden', requiredPermission: 'INCOME_CLIENT_READ' }} />;
    }
  ...
  async function save() {
    try {
      ...
      if (isEdit) {
        await incomeService.updateClient(edit.id, {
          code,
          name,
          email,
          phone,
        ...
        await incomeService.createClient({ code, name, email, phone });
      }
    ...
    <div className="p-6">
      <div className="bg-white rounded-xl shadow p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Revenue Clients</h2>
            <p className="text-sm text-slate-500 mt-1">Manage clients.</p>
          </div>
        ...
        {rows.length === 0 ? (
          <div className="mt-6">
            <EmptyState title="No clients" description="Create your first client." />
          </div>
        ) : (
            ...
            <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-xl p-6">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-sm text-slate-500 mt-1">Add/Edit</div>
                </div>
              ...
              <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={code} onChange={(e) => setCode(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="ACME" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600">Name</label>
                  <input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Acme Corp" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600">Email</label>
                  <input value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="billing@acme.com" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600">Phone</label>
                  <input value={phone} onChange={(e) => setPhone(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="9999999999" />
                </div>
              </div>
