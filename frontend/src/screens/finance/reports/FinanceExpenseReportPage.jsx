...
export function FinanceExpenseReportPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        setStatus('loading');
        setError(null);
        const payload = await reportingService.expense({
          from: range.from,
          to: range.to,
          groupBy: 'DIVISION'
        });
  ...
  return (
    <ReportPageShell
      title="Expense"
      subtitle={status === 'forbidden' ? (error?.message || 'Access denied') : 'Expense summary (live)'}
      status={status}
      canAccess={canAccess}
